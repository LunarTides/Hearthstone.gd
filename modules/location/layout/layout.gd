...
func handler(what: Modules.Hook, info: Array) -> bool:
	if what == Modules.Hook.CARD_HOVER_START:
		return hover_start_hook.callv(info)
	elif what == Modules.Hook.CARD_HOVER_STOP:
		return hover_stop_hook.callv(info)
...
#region Hooks
func hover_start_hook(card: Card) -> bool:
	if _layout_tweens.has(card.get_rid()):
		_layout_tweens[card.get_rid()].kill()
...
func hover_stop_hook(card: Card) -> bool:
	layout(card)
	return true
...
func card_play_before_hook(card: Card) -> bool:
	_should_layouts[card.get_rid()] = true
	return true
