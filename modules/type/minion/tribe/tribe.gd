...
const TRIBE_LABEL_SCENE: PackedScene = preload("res://modules/type/minion/tribe/tribe_label.tscn")
#endregion
...
func _ready() -> void:
	Modules.register(&"Tribe", [&"Minion"], func() -> void:
		# Load module.
		Modules.register_hooks(&"Tribe", self.handler)
		Modules.register_card_mesh(&"Tribe", TRIBE_MESH, Vector3(0, 0.2, 2))
	...
	, func() -> void:
		...
		Modules.unregister_card_mesh(&"Tribe")
...
func blueprint_create_hook(blueprint: Blueprint) -> bool:
	var tribe_label: Label3D = TRIBE_LABEL_SCENE.instantiate()
...
func card_update_hook(card: Card) -> bool:
	...
	card.get_node("Mesh/Tribe").visible = tribe_visible
